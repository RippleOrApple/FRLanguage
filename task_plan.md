...
- [ ] 阶段 27：继续减少自举验证对 Python 测试脚手架的依赖，补更强的错误传播和更复杂目标程序。
  - [x] 阶段 27.1：让 FR 自解释器按文件路径运行目标程序，并让目标 import 按当前文件目录解析。
  - [x] 阶段 27.2：新增二层自举验收，验证目标 FR 程序导入 FR bootstrap 后能跑默认 15 个用例。
  - [x] 阶段 27.3：补更完整的错误停止策略，让自解释器遇到运行时错误时更接近 Python 实现。
  - [ ] 阶段 27.4：评估执行模型优化，降低深层自举对宿主递归栈的依赖。
  - [ ] 阶段 27.5：补 Future reject、错误定位和更细的错误类型。
    - [x] 阶段 27.5.1：补自举 Future rejected 状态，并让 await 传播 Future body 中的运行时错误。
    - [ ] 阶段 27.5.2：补错误定位和更细的错误类型。
...
- 默认 bootstrap 验收集合先覆盖 18 个目标程序，包含正常输出路径、Future reject、错误停止和错误诊断路径，并由 FR 代码自己计算 `passed_count` / `failed_count`。
- 自解释器执行目标文件时需要保存当前文件目录，否则目标程序内部的同级 import 会错误地按宿主 examples 根目录解析。
- 当前树遍历解释器会让二层自举产生很深的宿主调用栈；短期通过提高 Python 递归上限支撑学习实验，长期应考虑 VM 或显式调用栈。
- 错误停止策略采用 `ErrorSignal` Map 在自解释器内部传播；它先解决继续执行问题，不引入 FR 语言级异常语法。
- 自举 Future 的 rejected 状态先保存错误消息字符串，足够让 await 重放失败；后续再补更结构化的错误对象。
