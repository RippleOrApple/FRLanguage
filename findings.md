...
- `runDefaultBootstrapExpectations()` 把 expected 输出和 expected 错误放进 FR 工具链本身，当前默认 18 个用例能覆盖主要正常路径、Future reject、错误停止和基础错误路径。
...
- FR 自解释器现在可以加载 `toolchain/bootstrap.fr`，也可以执行一个目标 FR 程序，由该目标程序导入 FR bootstrap 并跑默认 18 个验收用例。
...
- 自举 Future 现在有 `rejected` 状态和 `error` 字段；Future body 中的运行时错误会在 await 时停止外层执行，并保留 Future 自身失败状态。
