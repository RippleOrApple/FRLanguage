...
这个闭环暂时覆盖变量声明、`print`、字面量、变量读取、括号、一元表达式、基础二元表达式、List/Map 字面量、索引读取、变量赋值和索引赋值。
...
- FR Parser 子集还没有覆盖 `if`、`while`、函数声明、函数调用和 `return`。
- FR 解释器子集还没有覆盖作用域、控制流、函数、Future 和运行时错误诊断。
- 模块系统还没有命名空间和导出控制。
...
- 扩展 FR Parser 子集：优先补 `if` 和 `while`，让自举程序能表达更复杂控制流。
- 扩展 FR 解释器子集：优先补作用域和控制流，再考虑函数调用。
