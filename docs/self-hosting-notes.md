...
examples/fr_parser_future_reject_sample.fr.txt
...
这个闭环暂时覆盖变量声明、`print`、目标程序 `import`、字面量、变量读取、括号、一元表达式、基础二元表达式、`and/or` 短路逻辑、List/Map 字面量、索引读取、变量赋值、索引赋值、`if`、`while`、`break`、block 局部作用域、函数声明、函数调用、常用内置函数桥接、闭包读取、跨函数全局调用、递归、`return`、最小 `future/await`、Future reject、基础运行时错误诊断、运行时错误停止和非法控制流诊断。

`examples/fr_bootstrap_acceptance.fr` 会调用 `runDefaultBootstrapExpectations()`，默认运行 18 个自举验收用例，并返回：
...
Python 写的 FR 解释器
  ...
  -> 内层 FR bootstrap 跑默认 18 个验收用例
```

对应测试会检查输出为 `true`、`18`、`0`，表示内层默认验收全部通过。为了支撑这种深层树遍历调用，Python 解释器初始化时会把宿主递归上限提升到一个学习项目可接受的范围。
...
- 自举 Future 目前是最小 Map 模型，已经支持 `pending`、`resolved`、`rejected` 状态和 await 时传播失败；但还没有复刻 Python Runtime 队列和更完整的异步调度。
- FR 解释器子集已经能在基础运行时错误后停止继续执行，但错误定位和错误类型还不完整。
...
- 扩展 FR 解释器子集：补更完整的错误定位、错误类型和非法控制流边界。
