...
```fr
let source = readFile("examples/hello.fr");
print(source);
```
...
1. 用 FR 写字符串扫描 demo。
2. 用 FR 写简化版 Lexer。
3. 用 FR 写 Token 格式化工具。
4. 用 FR 写简单表达式 Parser。
...
- List 已经完成，下一步需要 Map 表达 Token、配置和结构化数据。
- 字符串内置函数是未来用 FR 写 Lexer 的关键。
- Map 和字符串能力完成后，FR 就更接近能写工具链小 demo。

Map 和字符串内置函数完成后，项目就会明显进入“能用 FR 写工具”的阶段。
