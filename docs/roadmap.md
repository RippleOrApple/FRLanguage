...
- FR Parser/Interpreter 子集应逐步扩展，当前已覆盖目标程序 `import`、集合字面量、索引读写、逻辑短路、控制流、`break`、函数、常用内置函数桥接、block 局部作用域、闭包读取、跨函数全局调用、最小 `future/await`、基础运行时错误诊断和非法控制流诊断，下一步重点是补更完整错误传播。
