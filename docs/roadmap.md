...
- FR Parser/Interpreter 子集应逐步扩展，当前已覆盖集合字面量、索引读写、逻辑短路、控制流、`break`、函数、常用内置函数桥接、block 局部作用域、闭包读取、跨函数全局调用和最小 `future/await`，下一步重点是补运行时错误诊断和更正式的工具链组织。
...
- 当前自解释器已经具备环境链、函数闭包读取、break、常用内置函数桥接和最小 Future。
