...
- FR Parser/Interpreter 子集应逐步扩展，当前已覆盖目标程序 `import`、集合字面量、索引读写、逻辑短路、控制流、`break`、函数、常用内置函数桥接、block 局部作用域、闭包读取、跨函数全局调用、最小 `future/await`、Future reject、基础运行时错误诊断、运行时错误停止和非法控制流诊断；`bootstrap.fr` 已能用 18 个默认用例做 expected 验收，并且二层自举目标程序已经能导入 FR bootstrap 后跑默认验收。
...
- 当前已有 `examples/fr_nested_bootstrap_acceptance.fr.txt` 验证二层自举：FR 自解释器执行一个目标 FR 程序，该目标程序再导入 FR bootstrap 并运行默认 18 个验收用例。
...
- 下一块应继续补错误定位、更细错误类型，或减少深层自举对宿主递归栈的依赖。
